...
export default function ConnectedRepositories() {
  ...
  // ── Fetch analyzed repos from backend 
  const fetchRepos = async () => {
    setLoading(true);
    try {
      const res = await api.get("/analysis/history?limit=50");
      setRepos(res.data.history ?? []);
    } catch (err: any) {
  ...
  // ── Disconnect a single analysis instance
  const handleDisconnect = async (analysisId: number, repoName: string) => {
    ...
    try {
      ...
      setRepos(prev => prev.filter(r => r.analysis_id !== analysisId));
      showToast(`"${repoName}" disconnected`, "success");
    } catch {
  ...
  return (
    <DashboardLayout>
      <style>{`
        ...
        .repo-row { transition: background 0.15s; }
        .repo-row:hover { background: rgba(255,255,255,0.02); border-radius: 12px; }
      ...
      <div style={{ padding: "32px 36px", maxWidth: "720px", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "28px" }}>
          ...
          <div>
            <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "24px", fontWeight: 800, color: "white", margin: 0, letterSpacing: "-0.4px" }}>
              Connected Repositories
            </h1>
            <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.3)", margin: "2px 0 0" }}>
              {loading ? "Loading…" : `${completedRepos.length} analyzed ${completedRepos.length === 1 ? "repository" : "repositories"}`}
            </p>
          </div>
        </div>
        ...
        <Card style={{ marginBottom: "16px" }}>
          ...
          {!loading && completedRepos.length === 0 && (
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              ...
              <div style={{ fontSize: "14px", fontWeight: 600, color: "rgba(255,255,255,0.35)", marginBottom: "4px" }}>
              ...
              <button
                onClick={() => navigate(analysisDashPath)}
                style={{
          ...
          {!loading && completedRepos.map((repo) => {
            ...
            const isDisconnecting = disconnecting === repo.analysis_id;

            return (
              <div
                key={repo.analysis_id}
                className="repo-row"
                style={{ display: "flex", alignItems: "center", gap: "14px", padding: "14px 8px", borderBottom: "1px solid rgba(255,255,255,0.04)" }}
              >
                {/* Repo icon */}
                <div style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: "14px", fontWeight: 600, color: "white", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: "3px" }}>
                    {repo.repo_name}
                  </div>
                  <div style={{ fontSize: "11.5px", color: "rgba(255,255,255,0.3)" }}>
                    {repo.branch} · {repo.triggered_at ? timeAgo(repo.triggered_at) : "—"}
                  </div>
                </div>
                ...
                {/* View button — navigates to /analysis/:analysisId */}
                <button
                  className="view-btn"
                  onClick={() => navigate(`/analysis/${repo.analysis_id}`)}
                  style={{
                ...
                <button
                  className="disc-btn"
                  onClick={() => handleDisconnect(repo.analysis_id, repo.repo_name)}
                  disabled={isDisconnecting}
                  title="Disconnect this analysis"
                  style={{
