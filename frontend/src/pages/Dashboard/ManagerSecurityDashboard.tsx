...
interface SecurityRepo { id: number; name: string | null; full_name: string | null; is_private: boolean; last_analyzed_at: string | null; security_score: number; total_issues: number; }
interface RiskBreakdown { high: number; medium: number; low: number; total: number; }
interface TrendPoint { period: string; label: string; high: number; medium: number; low: number; }
interface CommonIssue { title: string; severity: string; occurrences: number; repositories_affected: number; }
interface SecurityMember { id: number; full_name: string; username: string; avatar_url: string | null; specialization: string | null; repository_count: number; security_score: number; high: number; medium: number; low: number; }
interface TeamSecurityOverview { overall_score: number; repository_count: number; total_issues: number; team_members: number; risk_breakdown: RiskBreakdown; trend: TrendPoint[]; common_issues: CommonIssue[]; systemic_risk_analysis: string; why_this_matters: string[]; members: SecurityMember[]; }
interface RepositoryRiskItem { repository_id: number; repository_name: string; high: number; medium: number; low: number; total_issues: number; risk_weight: number; security_score: number | null; }
interface RepositoryRiskResponse { period: string | null; label: string | null; repositories: RepositoryRiskItem[]; }
...
const emptyRepositoryRisk: RepositoryRiskResponse = { period: null, label: null, repositories: [] };
...
function Panel({ title, icon: Icon, children, className = "" }: { title: string; icon?: LucideIcon; children: ReactNode; className?: string }) {
  return (
    <section className={`ms-panel ${className}`}>
      <div className="ms-panel-title">{Icon && <Icon size={18} strokeWidth={2.2} />}<h2>{title}</h2></div>
      {children}
    </section>
  );
}

function ScoreHero({ score, repositoryCount, totalIssues, teamMembers }: { score: number; repositoryCount: number; totalIssues: number; teamMembers: number }) {
  return (
    <section className="ms-hero">
      ...
      <div className="ms-score-ring" style={{ "--score": `${Math.max(0, Math.min(100, score)) * 3.6}deg` } as CSSProperties}><Shield size={46} /></div>
    </section>
  );
}
...
function RiskCard({ label, value, total, severity }: { label: string; value: number; total: number; severity: "High" | "Medium" | "Low" }) {
  const meta = severityMeta[severity];
  const pct = total ? Math.min(100, (value / total) * 100) : 0;
  return (
    <div className="ms-risk-card" style={{ borderColor: meta.color, background: meta.bg }}>
      <div className="ms-risk-top">
        <span style={{ background: meta.bg, color: meta.color }}>{severity === "High" ? <ShieldAlert size={16} /> : <AlertTriangle size={16} />}</span>
        <strong>{value}</strong><small>issues</small>
      </div>
...
function IssueRow({ issue }: { issue: CommonIssue }) {
  ...
  return (
    <article className="ms-issue-row">
      ...
      <div><strong>{issue.title}</strong><p>{issue.occurrences} occurrence{issue.occurrences === 1 ? "" : "s"} • {issue.repositories_affected} repositor{issue.repositories_affected === 1 ? "y" : "ies"} affected</p></div>
...
function MemberSecurityRow({ member }: { member: SecurityMember }) {
  ...
  return (
    <article className="ms-member-row">
      ...
      <div className="ms-member-score" style={{ color }}><strong>{fmt(member.security_score)}</strong><span>/100</span></div>
    </article>
  );
}
  ...
  return (
    <article className="ms-contributor-card">
      <div className="ms-contributor-head">
        ...
        <div><strong>{contributor.full_name}</strong><p>{fmt(contributor.security_score)} security score</p></div>
...
function RepositoryRiskChart({ data }: { data: RepositoryRiskResponse }) {
  ...
  const segments = [
    { key: "high" as const, label: "High", weight: 5, color: severityMeta.High.color },
    { key: "medium" as const, label: "Medium", weight: 3, color: severityMeta.Medium.color },
    { key: "low" as const, label: "Low", weight: 1, color: severityMeta.Low.color },
  ];
  if (!data.repositories.length) return <div className="ms-repo-risk-empty">No repository risk data available yet.</div>;
  return (
        ...
        {data.repositories.map(repo => (
          <article className="ms-repo-risk-row" key={repo.repository_id}>
            ...
            <div className="ms-repo-risk-track" aria-label={`${repo.repository_name} risk score ${repo.risk_weight}`}>
              {segments.map(segment => { const count = repo[segment.key]; const weighted = count * segment.weight; return <span key={segment.key} title={`${segment.label}: ${count}`} style={{ width: `${(weighted / maxRisk) * 100}%`, background: segment.color }} />; })}
            </div>
            <div className="ms-repo-risk-scores"><strong>Risk Score {repo.risk_weight}</strong><span>Security Score {repo.security_score === null ? "N/A" : fmt(repo.security_score, 1)}</span></div>
          </article>
        ))}
      </div>
      <div className="ms-repo-risk-legend">{segments.map(segment => (<span key={segment.key}><i style={{ background: segment.color }} />{segment.label}</span>))}</div>
    </div>
  );
}
...
function CustomSelect({ value, onChange, options }: { value: string; onChange: (val: string) => void; options: { value: string; label: string }[] }) {
  ...
  return (
    <div ref={ref} className="ms-custom-select" style={{ position: "relative", minWidth: 250 }}>
      <button type="button" className="ms-custom-select-trigger" onClick={() => setOpen(prev => !prev)} aria-haspopup="listbox" aria-expanded={open}>
        <span>{selected?.label}</span>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" style={{ transition: "transform 0.2s", transform: open ? "rotate(180deg)" : "rotate(0deg)", flexShrink: 0 }}>
      ...
      {open && (
        <ul className="ms-custom-select-menu" role="listbox">
          {options.map(opt => (
            <li key={opt.value} role="option" aria-selected={opt.value === value} className={`ms-custom-select-option${opt.value === value ? " selected" : ""}`} onClick={() => { onChange(opt.value); setOpen(false); }}>
...
export default function ManagerSecurityDashboard() {
  ...
  const fetchRepos = async () => { const response = await api.get<SecurityRepo[]>("/manager/security/repos"); setRepos(response.data || []); };
  const fetchRepositoryRisk = async () => { const response = await api.get<RepositoryRiskResponse>("/manager/security/repository-risk"); setRepositoryRiskData(response.data || emptyRepositoryRisk); };
  ...
  useEffect(() => { fetchRepos().catch(console.error); }, []);
  useEffect(() => { fetchDashboard(); if (selectedRepoId === "all") fetchRepositoryRisk().catch(console.error); }, [selectedRepoId]);
  const activeBreakdown = selectedRepoId === "all" ? teamData.risk_breakdown : { high: repoData?.repository.high || 0, medium: repoData?.repository.medium || 0, low: repoData?.repository.low || 0, total: repoData?.repository.total_issues || 0 };
  ...
  return (
    <DashboardLayout>
      <main className="ms-page">
        <div className="ms-shell">
          <header className="ms-header">
            <div><h1>Team Security Health</h1><p>Security risk assessment and release readiness across team repositories</p></div>
            <button className="ms-refresh" type="button" onClick={() => { fetchRepos(); fetchRepositoryRisk(); fetchDashboard(); }} title="Refresh dashboard"><RefreshCcw size={17} /></button>
          </header>
          ...
          <section className="ms-filter">
            ...
            <CustomSelect value={selectedRepoId} onChange={setSelectedRepoId} options={[{ value: "all", label: "All Repositories (Team View)" }, ...repos.map(repo => ({ value: String(repo.id), label: titleForRepo(repo) }))]} />
          </section>
          ...
          {loading ? (
            ...
            <>
              ...
              <Panel title="Most Common Security Issues">
                <div className="ms-list">{teamData.common_issues.length ? teamData.common_issues.map(issue => <IssueRow key={issue.title} issue={issue} />) : <p className="ms-empty">No security issues detected.</p>}</div>
              </Panel>
      ...
      <style>{`
        ...
        /* ─── Base layout ─── */
        .ms-page { min-height: 100vh; max-width: none; margin: 0; padding: 32px 40px 80px; color: var(--text-primary); font-family: 'Inter', system-ui, sans-serif; background: var(--bg-gradient); }
        .ms-shell { max-width: 1180px; margin: 0 auto; display: flex; flex-direction: column; gap: 0; }
        .ms-header { display: flex; justify-content: space-between; gap: 18px; align-items: flex-start; margin-bottom: 28px; flex-wrap: wrap; }
        .ms-header h1 { margin: 0 0 6px; font-size: 30px; font-weight: 800; font-family: 'Inter', sans-serif; letter-spacing: -0.5px; line-height: 1.1; }
        .ms-header p, .ms-subtle { margin: 0; color: var(--text-muted); font-size: 14px; }

        /* ─── Refresh button ─── */
        .ms-refresh { width: 42px; height: 42px; border: 1px solid var(--border); border-radius: 12px; background: var(--bg-card); color: var(--text-primary); display: inline-flex; align-items: center; justify-content: center; cursor: pointer; transition: border-color 0.2s, background 0.2s, color 0.2s; }
        .ms-refresh:hover { border-color: #8b5cf680; background: #8b5cf612; color: #8b5cf6; }

        /* ─── Cards / panels ─── */
        .ms-hero, .ms-panel, .ms-filter, .ms-callout, .ms-about { border: 1px solid var(--border); background: var(--bg-card); border-radius: 18px; box-shadow: var(--shadow-card); }
        .ms-panel { padding: 30px 34px; margin-bottom: 18px; }
        .ms-panel-title { display: flex; align-items: center; gap: 9px; margin-bottom: 18px; color: #8b5cf6; }
        .ms-panel-title h2, .ms-callout h2 { margin: 0; font-size: 16px; font-weight: 850; color: var(--text-primary); }

        /* ─── Hero ─── */
        .ms-hero { display: flex; justify-content: space-between; gap: 24px; align-items: center; padding: 30px 34px; margin-bottom: 18px; border-color: rgba(139,92,246,0.28); background: linear-gradient(135deg, rgba(139,92,246,0.12), rgba(236,72,153,0.06)); }
        [data-theme="dark"] .ms-hero { background: linear-gradient(135deg, rgba(139,92,246,0.22), rgba(236,72,153,0.12)); }
        .ms-hero-label { display: flex; align-items: center; gap: 9px; font-weight: 800; color: #8b5cf6; }
        ...
        .ms-mini { border: 1px solid rgba(139,92,246,0.28); border-radius: 14px; padding: 12px; min-width: 0; background: var(--bg-input, var(--bg-card)); }
        .ms-mini strong { display: block; font-size: 19px; color: var(--text-primary); }
        .ms-mini span { color: var(--text-muted); font-size: 11.5px; }
        .ms-score-ring { --score: 0deg; position: relative; width: 108px; height: 108px; border-radius: 50%; background: conic-gradient(#8b5cf6 var(--score), rgba(139,92,246,0.2) 0); display: grid; place-items: center; color: #8b5cf6; flex: 0 0 auto; }
        ...
        /* ─── Filter bar ─── */
        .ms-filter { display: flex; justify-content: space-between; align-items: center; gap: 18px; padding: 16px 18px; margin-bottom: 18px; border-color: rgba(139,92,246,0.24); background: linear-gradient(135deg, rgba(139,92,246,0.08), rgba(236,72,153,0.04)); }
        .ms-filter div { display: flex; align-items: center; gap: 10px; color: #8b5cf6; }
        .ms-filter span { display: grid; color: var(--text-primary); }
        .ms-filter small { color: var(--text-secondary); font-size: 11px; }

        /* ─── Custom select — FULLY THEME-AWARE ─── */
        .ms-custom-select-trigger {
          display: flex; align-items: center; justify-content: space-between; gap: 10px;
          min-width: 250px; width: 100%; padding: 10px 14px;
        ...
        .ms-custom-select-trigger:hover { border-color: rgba(139,92,246,0.5); }
        .ms-custom-select-trigger:focus-visible { outline: none; border-color: rgba(139,92,246,0.6); box-shadow: 0 0 0 4px rgba(139,92,246,0.12); }
        ...
        .ms-custom-select-menu {
          ...
          z-index: 50; padding: 5px; margin: 0; list-style: none;
        }
        ...
        .ms-custom-select-option:hover { background: var(--bg-nav-hover); color: var(--text-primary); }
        .ms-custom-select-option.selected { color: #8b5cf6; font-weight: 700; background: var(--bg-nav-active); }
        .ms-custom-select-option.selected svg { color: #8b5cf6; flex-shrink: 0; }
        .ms-custom-select-option span { flex: 1; }

        /* ─── Risk cards ─── */
        .ms-risk-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }
        .ms-risk-card { border: 1px solid; border-radius: 14px; padding: 16px; }
        ...
        .ms-risk-top strong { font-size: 25px; color: var(--text-primary); }
        .ms-risk-top small { color: var(--text-muted); }
        .ms-risk-card b { display: block; margin: 14px 0 8px; font-size: 12px; }
        .ms-risk-bar { height: 5px; border-radius: 999px; background: var(--border); overflow: hidden; }
        ...
        .ms-issue-row, .ms-vuln-row, .ms-member-row { display: grid; grid-template-columns: auto 1fr auto auto; gap: 14px; align-items: center; border: 1px solid var(--border); border-radius: 14px; padding: 14px; background: var(--bg-card-hover); }
        ...
        .ms-callout.green { border-color: rgba(34,197,94,0.24); background: linear-gradient(135deg, rgba(34,197,94,0.08), rgba(20,184,166,0.04)); color: #22c55e; }
        ...
        .ms-contributor-card { border: 1px solid var(--border); border-radius: 14px; padding: 15px; background: var(--bg-card-hover); }
        ...
        .ms-repo-risk-track { display: flex; height: 18px; min-width: 0; overflow: hidden; border-radius: 999px; background: var(--border); }
        ...
        /* ─── About / misc ─── */
        .ms-about { display: flex; gap: 12px; padding: 16px 18px; margin-top: 4px; color: var(--text-muted); border-color: var(--border); background: var(--bg-card); }
