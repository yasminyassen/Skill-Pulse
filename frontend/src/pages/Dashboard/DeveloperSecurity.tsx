  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: size * 0.14, color: "var(--text-muted)" }}>/ 100</span>
      </div>
    </div>
  );
}
...
function EmptyCard({ title, body, accent }: { title: string; body: string; accent: string }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: "15px", fontWeight: 700, color: "var(--text-secondary)", marginBottom: "6px" }}>{title}</div>
      <div style={{ fontSize: "13px", color: "var(--text-muted)" }}>{body}</div>
    </div>
  );
}
...
function SecurityNotice({ selected, repoName, accent }: { selected: boolean; repoName?: string; accent: string }) {
  ...
  return (
    <div style={{
      ...
      padding: "18px 20px", background: `${accent}0F`,
      border: `1px solid ${accent}35`, borderRadius: "14px", marginBottom: "24px",
    }}>
      <div style={{
        width: "34px", height: "34px", borderRadius: "10px", background: `${accent}18`,
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: "12.5px", color: "var(--text-muted)", lineHeight: 1.6 }}>{body}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <DashboardLayout>
      <style>{`
        ...
        .sp-select option { background: var(--bg-card); color: var(--text-primary); }
        ...
        .sec-dim-card:hover { border-color: var(--border-hover); }
      ...
      <div style={{
        ...
        <div style={{ marginBottom: "32px" }}>
          ...
          <h1 style={{
            ...
            color: "var(--text-primary)", letterSpacing: "-0.5px", margin: "0 0 6px",
          }}>
            Security Overview
          </h1>
        ...
        <div style={card}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "28px" }}>
            <div>
              <div style={{ fontSize: "14px", fontWeight: 600, color: "var(--text-secondary)", marginBottom: "2px" }}>
                Security Score
              </div>
            ...
            <div style={{ textAlign: "right" }}>
              {loadingRepos ? (
                ...
                <>
                  <div style={{
                    fontSize: "42px", fontWeight: 800, color: "var(--text-primary)",
        ...
        <div style={{
          ...
          <span style={{ fontSize: "13.5px", color: "var(--text-secondary)", fontWeight: 500, whiteSpace: "nowrap" }}>
          ...
          <>
            {loadingDetail ? (
              ...
              <>
                      ...
                      {[
                        ...
                        <div key={item.label}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
                            <span style={{ fontSize: "12.5px", color: "var(--text-secondary)", fontWeight: 500 }}>{item.label}</span>
                            <span style={{ fontSize: "14px", fontWeight: 700, color: "var(--text-primary)" }}>{Math.round(item.value)}</span>
                          </div>
                          <div style={{ fontSize: "11px", color: "var(--text-muted)", marginBottom: "6px" }}>{item.sub}</div>
                ...
                {/* ── Findings breakdown ── */}
                <div className="sec-dim-card">
                  <div style={{ fontSize: "16px", fontWeight: 700, color: "var(--text-primary)", marginBottom: "18px" }}>
                    Findings Breakdown
                  </div>
                    ...
                    {(["HIGH", "MEDIUM", "LOW"] as const).map(severity => {
                      ...
                      return (
                        <div key={severity} style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, borderRadius: "12px", padding: "16px" }}>
                          <div style={{ fontSize: "24px", fontWeight: 900, color: "var(--text-primary)", marginBottom: "3px" }}>{count}</div>
                  ...
                  <div className="sec-metrics-grid">
                    <div>
                      ...
                      {topOwasp.length ? topOwasp.map(([name, count]) => (
                        <div key={name} style={{ display: "flex", justifyContent: "space-between", gap: "12px", fontSize: "12px", color: "var(--text-secondary)", padding: "6px 0", borderBottom: "1px solid var(--border)" }}>
                          <span>{name}</span>
                          <strong style={{ color: "var(--text-primary)" }}>{count}</strong>
                        </div>
                      )) : <span style={{ fontSize: "12px", color: "var(--text-muted)" }}>No OWASP categories detected.</span>}
                    </div>
                    <div>
                      <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--text-muted)", letterSpacing: "0.8px", textTransform: "uppercase", marginBottom: "10px" }}>
                        Top Affected Files
                      ...
                      )) : <span style={{ fontSize: "12px", color: "var(--text-muted)" }}>No affected files detected.</span>}
                    </div>
                  </div>
                </div>
                ...
                <div className="sec-dim-card">
                  ...
                  {findings.length ? (
                    ...
                    <EmptyCard
                      ...
                      body="The selected analysis did not produce security findings for this contribution scope."
                      accent={accent}
                    />
                  )}
                </div>
                ...
                {detail?.ai_insights?.security_insights && (
                  <div className="sec-dim-card" style={{ borderColor: `${accent}35`, background: `${accent}0F` }}>
                    <div style={{ display: "flex", gap: "12px", alignItems: "flex-start" }}>
                      ...
                      <div>
                        <div style={{ margin: "0 0 8px", fontSize: "16px", fontWeight: 800, color: "var(--text-primary)" }}>
                          AI Security Summary
                        </div>
