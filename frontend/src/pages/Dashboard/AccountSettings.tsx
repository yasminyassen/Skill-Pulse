...
function Field({ label, value, onChange, placeholder, type = "text", readOnly = false, hint }: FieldProps) {
  return (
    <div>
      <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)", fontWeight: 700, marginBottom: "7px", letterSpacing: "0.6px", textTransform: "uppercase" }}>
        {label}
      </div>
      <input
        type={type}
        ...
        style={{
          ...
          border: `1px solid ${readOnly ? "rgba(255,255,255,0.06)" : "rgba(255,255,255,0.1)"}`,
      ...
      />
      {hint && <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.25)", marginTop: "5px" }}>{hint}</div>}
    </div>
  );
}
      ...
      <div style={{ padding: "32px 36px", maxWidth: "680px", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "28px" }}>
          <button
            ...
            style={{ width: 36, height: 36, borderRadius: "10px", border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.04)", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.4)", cursor: "pointer" }}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6" /></svg>
          </button>
          <div>
            <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "24px", fontWeight: 800, color: "white", margin: 0, letterSpacing: "-0.4px" }}>Account Settings</h1>
            <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.3)", margin: "2px 0 0" }}>Manage your account details</p>
          </div>
        </div>
        ...
        <Card style={{ marginBottom: "16px" }}>
          {loading ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              ...
              <div>
                <div style={{ fontSize: "18px", fontWeight: 800, color: "white" }}>{user.full_name}</div>
                <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)", marginTop: "2px" }}>@{user.username}</div>
                {user.role && (
        ...
        <Card style={{ marginBottom: "16px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "20px", paddingTop: "16px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <button
              onClick={handleSave}
              disabled={saving || loading}
              style={{
                padding: "9px 22px", borderRadius: "10px", border: "none",
                background: saving ? "rgba(99,102,241,0.5)" : accent,
        ...
        <Card style={{ marginBottom: "16px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderRadius: "12px", background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <div style={{ width: 36, height: 36, borderRadius: "10px", background: "rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.6)" }}>
              ...
              <div>
                <div style={{ fontSize: "14px", fontWeight: 700, color: "white" }}>GitHub</div>
                <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)" }}>
                  {loading ? "—" : (user?.github_connected
                    ? (user.github_login ? `Connected as @${user.github_login}` : "Connected")
                    : "Not connected")}
                </div>
              </div>
            </div>
            <span style={{
              ...
              background: user?.github_connected ? "rgba(52,211,153,0.1)" : "rgba(255,255,255,0.05)",
              color: user?.github_connected ? "#34d399" : "rgba(255,255,255,0.3)",
              border: `1px solid ${user?.github_connected ? "rgba(52,211,153,0.2)" : "rgba(255,255,255,0.08)"}`,
            }}>
        ...
        <Card style={{ border: "1px solid rgba(248,113,113,0.15)", background: "rgba(248,113,113,0.03)" }}>
          ...
          <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.35)", margin: "0 0 16px" }}>
            Once you delete your account, all your data will be permanently removed. This action cannot be undone.
          </p>
          {!showDelete ? (
            <button
              onClick={() => setShowDelete(true)}
              style={{ padding: "9px 20px", borderRadius: "10px", border: "1px solid rgba(248,113,113,0.3)", background: "rgba(248,113,113,0.08)", color: "#f87171", fontSize: "13px", fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}
            >
              Delete My Account
            </button>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              <input
                ...
                style={{ padding: "9px 13px", borderRadius: "9px", background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.2)", color: "white", fontSize: "14px", outline: "none", fontFamily: "'DM Sans', sans-serif" }}
              />
              <div style={{ display: "flex", gap: "8px" }}>
                <button
                  onClick={() => { setShowDelete(false); setDeleteConfirm(""); }}
                  style={{ padding: "8px 16px", borderRadius: "9px", border: "1px solid rgba(255,255,255,0.08)", background: "transparent", color: "rgba(255,255,255,0.4)", fontSize: "13px", fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}
                >
                  Cancel
                </button>
                <button
                  ...
                  style={{ padding: "8px 20px", borderRadius: "9px", border: "none", background: deleteConfirm === "DELETE" ? "#ef4444" : "rgba(248,113,113,0.2)", color: "white", fontSize: "13px", fontWeight: 700, cursor: deleteConfirm === "DELETE" ? "pointer" : "not-allowed", fontFamily: "'DM Sans', sans-serif" }}
                >
